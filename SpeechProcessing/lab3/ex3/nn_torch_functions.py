...
class SleepinessDataset(Dataset):
    ...
    def __init__(self, data_files, label_files):
        ...
        # Makes sure that the labels are alligned with the explanatory variables
        y_train, y_dev = load_labels(label_files, [X_train, X_dev])
        ...
        '''
        self.train_names = X_train['name']
        self.dev_names = X_dev['name']
        self.test_names = X_test['name']

        X_train.drop(labels=['name', 'frameTime', 'file_id'], axis=1, inplace=True)
        X_dev.drop(labels=['name', 'frameTime', 'file_id'], axis=1, inplace=True)
...
def train(dataset, model, optimizer, scheduler, criterion, batch_size, epochs):
