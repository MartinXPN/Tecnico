...
from facefive.app import app
from facefive.app import model
...
def home():
    if 'username' in session:
        ...
        logging.debug("user in homepage: (%s)" % username)
        try:
            posts_to_show = model.get_all_posts(username)
        except Exception as e:
            logging.debug("home: Found exception(%s)" % e)
            return error("Error - could not complete operation")
...
def update_profile():
    ...
    if not new_photo:
        ...
        new_photo_filename = secure_filename(f'{"".join(random.choices(string.ascii_lowercase + string.digits, k=8))}'
                                             f'_{username}_{new_photo.filename}')
        new_photo.save('facefive/app/' + app.config['photos_folder'] + new_photo_filename)
        if current_user.photo != app.config['default_photo']:
            os.remove('facefive/app/' + app.config['photos_folder'] + current_user.photo)
...
def edit_post():
    ...
    if request.method == 'GET':
        ...
        try:
            post = model.get_post(post_id, username)
        except Exception as e:
    ...
    try:
        new_post = model.edit_post(post_id, new_content, new_type, username)
    except Exception as e:
